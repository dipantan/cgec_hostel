...
export function insertRoomChange(data, callback) {
  connection.query(
    "select * from room_change_request where email = ? and status = 'pending'",
    [data.email],
    function (err, result) {
      if (err) {
        return callback(err, null);
        ...
        if (result.length > 0) {
          ...
          connection.query(
            "insert into room_change_request (from_room,to_room,reason,date,email,status) values (?,?,?,?,?,?)",
            [
              data.from_room,
              data.to_room,
              data.reason,
...
export function insertComplain(data, callback) {
  connection.query(
    "insert into complain (description,email,title) values (?,?,?)",
...
export function chnageMeal(data, callback) {
  connection.query(
    "insert into meal (`email`,`preference`,`from`,`meal_type`,`to`) values (?,?,?,?,?)",
